...
struct SettingsStoreTests {
    ...
    func `round trips every value`() {
        ...
        store.matchingRules = MatchingRules(genericTitles: ["Busy"], jitsiHosts: ["jitsi.example.org"])
        store.takeover = TakeoverSettings(onEventStart: false, onReminderDue: true, snoozeMinutes: [1, 2])
        store.titleLimit = 30
        ...
        #expect(reloaded.matchingRules.jitsiHosts == ["jitsi.example.org"])
